...
class BoundaryConditions:
    ...
    def get_boundary_conditions(self,state,targets):
        ...
        self.Pe = targets.Pe[-1] if targets.roa_eval[-1] == 1 else Exception("Invalid targets for BCs") # power
        self.Pi = targets.Pi[-1] if targets.roa_eval[-1] == 1 else Exception("Invalid targets for BCs")
...
class TwoPoint_EichManz(BoundaryConditions):
    """
    LCFS (Last Closed Flux Surface) boundary condition handler.
    
    Implements Eich-Goldston scaling, two-point model, and SOL physics
    to determine separatrix values and gradient scale lengths.
    ...
    def get_boundary_conditions(self, state) -> None:

        self.get_TwoPoint_y(state)
        self.get_EichManz_aLy(state)


    def get_EichManz_aLy(self, state) -> None:
        """
        Calculate gradient scale lengths at separatrix.
        
        Uses Eich-Goldston scaling for SOL width.
        ...
        -------
        aLy : dict
            {'ne': a/Lne, 'te': a/Lte, 'ni': a/Lni, 'ti': a/Lti}
        """
        # Eich-Goldston scaling
        kne_sep = max(0.0, 1.0 / (2.9 * (1 + 10.4 * state.alphat**2.5) *
                                   state.rhostar * state.B_T / state.B_p))
        kTe_sep = max(0.0, 1.0 / (2.1 * (1 + 2.1 * state.alpha**1.7) *
                                   state.rhostar * state.B_T / state.B_p))
        kTi_sep = max(0.0, self.aLTratio_sep * kTe_sep)
        kni_sep = max(0.0, 1.0 / (2.9 * (1 + 10.4 * state.alphat**2.5) *
                                   state.rhostar * state.B_T / state.B_p))
        
        #{'ne': kne_sep, 'te': kTe_sep, 'ni': kni_sep, 'ti': kTi_sep}
        state.BC.aLne = kne_sep
        state.BC.aLte = kTe_sep
        state.BC.aLni = kni_sep
        state.BC.aLti = kTi_sep 

    
    def get_TwoPoint_y(self, state) -> None:
        """
        Solve Stangeby two-point model for target plate conditions.
        
        Assumes conduction-limited SOL with:
            q_|| = -kappa_0 * T^(7/2) * dT/ds
        
        Parameters
        ----------
        Psol : float, optional
            SOL power [MW]. If None, uses self.Psol.
        ...
        #TODO: Implement Tratio_sep, ne, and ni handling through options
        Psol = state.Pe + state.Pi # assumes electrons carry most power to divertor
        te_sep = max(0.01,(Psol/1e-6)**(2/7)/(1e3*state.T_norm)) #self.Pe_2pt(ne,Te,ni,Ti)/(ne+ni)
        ti_sep = self.Tratio_sep*te_sep

        state.BC.ne = self.ne_sep
        state.BC.te = te_sep
        state.BC.ni = self.ni_sep
        state.BC.ti = ti_sep


BOUNDARY_CONDITION_MODELS = {
    'TwoFluid_EichManz': TwoPoint_EichManz,
