...
class Qlgyro(TransportBase):
    ...
    def _read_qlgyro_gbflux(self, path: Path) -> Tuple[float, float, float]:
        ...
        if data_flat.size % 4 != 0:
            raise RuntimeError(f"QLGYRO gbflux size {data_flat.size} not divisible by 4")
    ...
    def _read_qlgyro_exchange_spectrum(self, path: Path) -> float:
        """Read turbulent exchange from out.qlgyro.flux_spectrum."""
            ...
            for line in f:
                ...
                for token in s.split():
                    try:
                        ...
                        pass
                # QLGYRO flux_spectrum: ky + 5 flux channels, exchange in column 6.
                if len(vals) >= 6:
                    exchange_by_species[current_species] = exchange_by_species.get(current_species, 0.0) + vals[5]
    ...
    def _read_platform_results(self, work_dir: Path, state: "PlasmaState") -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        for i, rho in enumerate(roa_eval):
            ...
            exch_path = rho_dir / "out.qlgyro.flux_spectrum"
            ge_gB, qi_gB, qe_gB = self._read_qlgyro_gbflux(out_path)
            qie_gB = self._read_qlgyro_exchange_spectrum(exch_path)
            Ge_turb_gB[i] = ge_gB
            Qi_turb_gB[i] = qi_gB
            Qe_turb_gB[i] = qe_gB
