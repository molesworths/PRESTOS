...
inspired by MITIM's GEOMETRYtools.py.
"""

import numpy as np

def calculateGeometricFactors(cls, n_theta=1001):
    ...
    shape_coeffs = cls.shape_cos + cls.shape_sin

    # ----------------------------------------
    # Derivatives as defined in expro_util.f90
    # ----------------------------------------

    s_delta = r * np.gradient(delta,r)
    s_kappa = r / kappa * np.gradient(kappa,r)
    s_zeta = r * np.gradient(zeta,r)
    dzmag = np.gradient(zmag,r)
    dRmag = np.gradient(R,r)

    s_shape_coeffs = []
    for i in range(len(shape_coeffs)):
        if shape_coeffs[i] is not None:
            s_shape_coeffs.append(r * np.gradient(shape_coeffs[i],r))
        else:
    ...
    # Scale dimensionless Miller outputs to physical units using local minor radius r(i)
    # In GACODE (expro_util.f90): expro_volp(i) = geo_volume_prime * r_min(i)^2
    # Here, r_min(i) corresponds to cls.r[i] (in meters), not the edge radius a.
    volp = geo_volume_prime * (cls.a ** 2)
    surf = geo_surf * (cls.a ** 2)
